 ...
 ******************************************************************************/
bool
GLIB_rectContainsPoint(const GLIB_Rectangle_t *pRect, int32_t x, int32_t y)
{
	if ((pRect == NULL) || (x < pRect->xMin) || (x > pRect->xMax)
			|| (y < pRect->yMin) || (y > pRect->yMax))
	{
		return false;
	}
	return true;
...
EMSTATUS GLIB_drawRect(GLIB_Context_t *pContext, const GLIB_Rectangle_t *pRect)
{
	...
	status = GLIB_drawLineH(pContext, tmpRectangle.xMin, tmpRectangle.yMin,
			tmpRectangle.xMax);
	if (status != GLIB_OK)
	{
		return status;
	...
	status = GLIB_drawLineV(pContext, tmpRectangle.xMax, tmpRectangle.yMin + 1,
			tmpRectangle.yMax);
	if (status != GLIB_OK)
	{
		return status;
	...
	status = GLIB_drawLineH(pContext, tmpRectangle.xMin, tmpRectangle.yMax,
			tmpRectangle.xMax - 1);
	if (status != GLIB_OK)
	{
		return status;
	...
	return GLIB_drawLineV(pContext, tmpRectangle.xMin, tmpRectangle.yMin + 1,
			tmpRectangle.yMax - 1);
}
		...
		const GLIB_Rectangle_t *pRect)
{
	...
	status = DMD_setClippingArea(tmpRectangle.xMin, tmpRectangle.yMin, width,
			height);
	if (status != DMD_OK)
	{
		return status;
