...
static uint8_t
GLIB_getClipCode(GLIB_Context_t *pContext, int32_t x, int32_t y);
static bool
GLIB_clipLine(GLIB_Context_t *pContext, int32_t *pX1, int32_t *pY1,
		...
		int32_t x2)
{
	...
	if ((y1 < pContext->clippingRegion.yMin)
			|| (y1 > pContext->clippingRegion.yMax))
	{
	...
	if ((x1 > pContext->clippingRegion.xMax)
			|| (x2 < pContext->clippingRegion.xMin))
	{
		return GLIB_ERROR_NOTHING_TO_DRAW;
	}
		...
		int32_t y2)
{
	...
	/* Check if line is outside of clipping region */
	if ((x1 < pContext->clippingRegion.xMin)
			|| (x1 > pContext->clippingRegion.xMax))
	{
		return GLIB_ERROR_NOTHING_TO_DRAW;
	}
	...
	/* Check if entire line is outside clipping region */
	if ((y1 > pContext->clippingRegion.yMax)
			|| (y2 < pContext->clippingRegion.yMin))
	{
		return GLIB_ERROR_NOTHING_TO_DRAW;
	}
		...
		int32_t *pX2, int32_t *pY2)
{
	...
	while (true)
	{
		...
		if (currentCode & 1)
		{
			...
			y = *pY1
					+ ((*pY2 - *pY1) * (pContext->clippingRegion.xMin - *pX1))
							/ (*pX2 - *pX1);
		...
		else if (currentCode & 2)
		{
			...
			y = *pY1
					+ ((*pY2 - *pY1) * (pContext->clippingRegion.xMax - *pX1))
							/ (*pX2 - *pX1);
		}
		...
		else if (currentCode & 4)
		{
			...
			x = *pX1
					+ ((*pX2 - *pX1) * (pContext->clippingRegion.yMax - *pY1))
							/ (*pY2 - *pY1);
		}
		...
		else if (currentCode & 8)
		{
			...
			x = *pX1
					+ ((*pX2 - *pX1) * (pContext->clippingRegion.yMin - *pY1))
							/ (*pY2 - *pY1);
		}
