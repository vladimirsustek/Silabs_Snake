...
EMSTATUS
(*fpReadData)(uint8_t buffer[], uint32_t bufLength, uint32_t bytesToRead);

/** @endcond */

/* Local function declarations */
static EMSTATUS
BMP_readRawData8bit(BMP_DataType *dataType, uint8_t buffer[],
		uint32_t bufLength);
static EMSTATUS
BMP_readRawData24bit(BMP_DataType *dataType, uint8_t buffer[],
		uint32_t bufLength);
static EMSTATUS
BMP_readRawDataRLE8(BMP_DataType *dataType, uint8_t buffer[],
		uint32_t bufLength);
static EMSTATUS
BMP_readPaddingBytes(uint8_t paddingBytes);
static EMSTATUS
BMP_readRleData(BMP_DataType *dataType, uint8_t buffer[], uint32_t bufLength);
static EMSTATUS
BMP_readRgbDataRLE8(uint8_t buffer[], uint32_t bufLength, uint32_t *pixelsRead);
...
EMSTATUS BMP_reset()
{
	...
	{
		bytesInImage = (bmpHeader.imageDataSize / bmpHeader.height)
				* bmpHeader.height;
	}
	...
	if (bmpHeader.bitsPerPixel == 8)
	{
		...
		else if (bmpHeader.compressionType == NO_COMPRESSION)
		{
			...
			while (bytesLeftInBuffer > 0 && dataType.endOfRow == 0)
			{
				...
				status = BMP_readRawData8bit(&dataType, localCache,
						bytesToRead);
				if (status != BMP_OK)
				{
					return status;
				...
				for (i = 0; i < dataType.size; ++i)
				{
					...
					/* Set green */
					buffer[bufferIdx + 1] = bmpPalette.data[4 * localCache[i]
							+ 1];
					/* Set blue */
					buffer[bufferIdx + 2] = bmpPalette.data[4 * localCache[i]
							+ 2];
 ...
 *  Returns BMP_OK on success, or else error code
 ******************************************************************************/
static EMSTATUS BMP_readRgbDataRLE8(uint8_t buffer[], uint32_t bufLength,
		uint32_t *pixelsRead)
{
	EMSTATUS status;
	BMP_DataType dataType;
	...
	while (bytesLeftInBuffer > 0 && dataType.endOfRow == 0)
	{
		if (rleInfo.mode == MODE_RLE)
		{
			...
			while (rleInfo.pixelsRemaining > 0 && bytesLeftInBuffer > 0)
			{
				...
				/* Set green */
				buffer[bufferIdx + 1] =
						bmpPalette.data[4 * rleInfo.pixelIdx + 1];
				/* Set blue */
				buffer[bufferIdx + 2] =
						bmpPalette.data[4 * rleInfo.pixelIdx + 2];
 ...
 *  Returns BMP_OK on success
 ******************************************************************************/
static EMSTATUS BMP_readRawData8bit(BMP_DataType *dataType, uint8_t buffer[],
		uint32_t bufLength)
{
	/* Check if end of file is reached */
	if (dataIdx >= bytesInImage)
 ...
 *  Returns BMP_OK on success, or else error code
 ******************************************************************************/
static EMSTATUS BMP_readRawData24bit(BMP_DataType *dataType, uint8_t buffer[],
		uint32_t bufLength)
{
	/* Check if end of file is reached */
	if (dataIdx >= bytesInImage)
 ...
 *  Returns BMP_OK on success, or else error code
 ******************************************************************************/
static EMSTATUS BMP_readRawDataRLE8(BMP_DataType *dataType, uint8_t buffer[],
		uint32_t bufLength)
{
	EMSTATUS status;
	...
	if (rleInfo.mode == MODE_RLE)
	{
		...
		while (rleInfo.mode == MODE_RLE && bufferIdx < bufLength
				&& dataType->endOfRow == 0)
		{
			/* Read in 2 RLE bytes */
			status = BMP_readRleData(dataType, localCache,
	...
	if (rleInfo.mode == MODE_RLE && rleInfo.pixelsRemaining == 0
			&& dataType->endOfRow == 0)
	{
		/* Read in the next 2 bytes to see if end of row is reached or end of file */
...
static EMSTATUS BMP_readPaddingBytes(uint8_t paddingBytes)
{
	if (paddingBytes > 0)
	{
		...
		EMSTATUS status = fpReadData(localCache, BMP_LOCAL_CACHE_SIZE,
				paddingBytes);
		if (status != BMP_OK)
		{
			return status;
 ...
 ******************************************************************************/
static EMSTATUS BMP_readRleData(BMP_DataType *dataType, uint8_t buffer[],
		uint32_t bufLength)
{
	EMSTATUS status;
