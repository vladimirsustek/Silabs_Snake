...
static EMSTATUS
GLIB_drawPartialCirclePoints(GLIB_Context_t *pContext, int32_t xCenter,
		int32_t yCenter, int32_t x, int32_t y, uint8_t bitMask);
		...
		int32_t yCenter, uint32_t radius, uint8_t bitMask)
{
	...
	status = GLIB_drawPartialCirclePoints(pContext, xCenter, yCenter, x, y,
			bitMask);
		...
		int32_t yCenter, uint32_t radius)
{
	...
	while (x < y)
	{
		...
		status = GLIB_drawLineH(pContext, xCenter - x, yCenter + y,
				xCenter + x);
		if (status > GLIB_ERROR_NOTHING_TO_DRAW)
		{
			return status;
		...
		status = GLIB_drawLineH(pContext, xCenter - y, yCenter + x,
				xCenter + y);
		if (status > GLIB_ERROR_NOTHING_TO_DRAW)
		{
			return status;
...
static EMSTATUS GLIB_drawPartialCirclePoints(GLIB_Context_t *pContext,
		int32_t xCenter, int32_t yCenter, int32_t x, int32_t y, uint8_t bitMask)
{
	EMSTATUS status;
	uint32_t drawnElements = 0;
	uint32_t i;
	...
	while (bitMask)
	{
		...
		if (bitMask & 0x1)
		{
			status = GLIB_drawPixel(pContext, xCenter + xOffsets[i],
					yCenter + yOffsets[i]);
			if (status > GLIB_ERROR_NOTHING_TO_DRAW)
			{
				return status;
