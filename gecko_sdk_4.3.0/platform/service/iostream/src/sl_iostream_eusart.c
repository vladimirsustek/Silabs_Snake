...
static sl_status_t
eusart_tx(void *context, char c);

static void
eusart_set_next_byte_detect(void *context);
...
sl_status_t sl_iostream_eusart_init(sl_iostream_uart_t *iostream_uart,
		sl_iostream_uart_config_t *uart_config, EUSART_UartInit_TypeDef *init,
		...
		sl_iostream_eusart_context_t *eusart_context)
{
	...
	status = sli_iostream_uart_context_init(iostream_uart,
			&eusart_context->context, uart_config, eusart_tx,
#if defined(SL_CATALOG_POWER_MANAGER_PRESENT) && !defined(SL_IOSTREAM_UART_FLUSH_TX_BUFFER)
			eusart_tx_completed,
#else
			NULL,
#endif
			eusart_set_next_byte_detect, eusart_deinit, em_req, em_req);
	...
	GPIO_PinModeSet(eusart_config->tx_port, eusart_config->tx_pin,
			gpioModePushPull, 1);
	GPIO_PinModeSet(eusart_config->rx_port, eusart_config->rx_pin,
			gpioModeInputPull, 1);
...
#error Unsupported board configuration
...
#else
	GPIO->EUSARTROUTE[eusart_config->port_index].ROUTEEN =
	...
	if (cts == true)
	{
		GPIO_PinModeSet(eusart_config->cts_port, eusart_config->cts_pin,
				gpioModeInputPull, 0);
#if defined(EUART_COUNT) && (EUART_COUNT > 0)
    GPIO->EUARTROUTE_SET->CTSROUTE = (eusart_config->cts_port << _GPIO_EUART_CTSROUTE_PORT_SHIFT)
		...
		GPIO->EUSARTROUTE_SET[eusart_config->port_index].CTSROUTE =
				(eusart_config->cts_port << _GPIO_EUSART_CTSROUTE_PORT_SHIFT)
						| (eusart_config->cts_pin
								<< _GPIO_EUSART_CTSROUTE_PIN_SHIFT);
#endif
	}
	if (rts == true)
	{
		GPIO_PinModeSet(eusart_config->rts_port, eusart_config->rts_pin,
				gpioModePushPull, 0);
#if defined(EUART_COUNT) && (EUART_COUNT > 0)
...
#else
		GPIO->EUSARTROUTE_SET[eusart_config->port_index].ROUTEEN =
		...
		GPIO->EUSARTROUTE_SET[eusart_config->port_index].RTSROUTE =
				(eusart_config->rts_port << _GPIO_EUSART_RTSROUTE_PORT_SHIFT)
						| (eusart_config->rts_pin
								<< _GPIO_EUSART_RTSROUTE_PIN_SHIFT);
#endif
	}
...
{
	sl_iostream_eusart_context_t *eusart_context =
			(sl_iostream_eusart_context_t*) iostream_uart->stream.context;
      ...
      if (eusart_context->context.block) {
		...
		if (eusart_context->context.rx_buffer_full == true)
		{
			...
			if (eusart_context->context.sw_flow_control == true)
			{
				...
				{
					eusart_context->context.ctrl_char_scan_ptr =
							(uint8_t*) LDMA->CH[eusart_context->context.dma.channel].DST
									- 1;
				}
...
{
	sl_iostream_eusart_context_t *eusart_context =
			(sl_iostream_eusart_context_t*) context;
...
static sl_status_t eusart_deinit(void *context)
{
	...
	GPIO_PinModeSet(eusart_context->tx_port, eusart_context->tx_pin,
			gpioModeDisabled, 0);
	GPIO_PinModeSet(eusart_context->rx_port, eusart_context->rx_pin,
			gpioModeDisabled, 0);

	// De-Configure Flow Control GPIOs
	if (eusart_context->flags & SLI_IOSTREAM_UART_FLAG_CTS)
	{
		GPIO_PinModeSet(eusart_context->cts_port, eusart_context->cts_pin,
				gpioModeDisabled, 0);
	}
	if (eusart_context->flags & SLI_IOSTREAM_UART_FLAG_RTS)
	{
		GPIO_PinModeSet(eusart_context->rts_port, eusart_context->rts_pin,
				gpioModeDisabled, 0);
	}
