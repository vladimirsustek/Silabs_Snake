 ...
 ******************************************************************************/
static sl_status_t
uart_deinit(void *stream);

static sl_status_t
uart_write(void *context, const void *buffer, size_t buffer_length);

static sl_status_t
uart_read(void *context, void *buffer, size_t buffer_length, size_t *bytes_read);

static void
set_auto_cr_lf(void *context,
bool on);

static bool
get_auto_cr_lf(void *context);
...
static void
scan_for_ctrl_char(sl_iostream_uart_context_t *uart_context);

static sl_status_t
nolock_uart_write(void *context, const void *buffer, size_t buffer_length);

static bool
dma_irq_handler(unsigned int chan, unsigned int seq, void *user_param);

__STATIC_INLINE uint8_t*
get_write_ptr(const sl_iostream_uart_context_t *uart_context);

static void
update_ring_buffer(sl_iostream_uart_context_t *uart_context);

static size_t
read_rx_buffer(sl_iostream_uart_context_t *uart_context, uint8_t *buffer,
		size_t buffer_len);
		...
		uint8_t tx_em_req)
{
  ...
  NVIC_EnableIRQ(config->tx_irq_number);
	...
	ecode = DMADRV_PeripheralMemory(context->dma.channel,
			context->dma.cfg.peripheral_signal, context->rx_buffer,
			context->dma.cfg.src,
			true, context->rx_buffer_len, dmadrvDataSize1, dma_irq_handler,
			context);
...
static sl_status_t uart_deinit(void *stream)
{
	...
	sl_iostream_uart_context_t *uart_context =
			(sl_iostream_uart_context_t*) uart->stream.context;
...
static void scan_for_ctrl_char(sl_iostream_uart_context_t *uart_context)
{
  ...
  EFM_ASSERT(ecode == ECODE_OK);
	...
	{
		newest_byte = uart_context->rx_buffer
				+ (uart_context->rx_buffer_len - 1);
	}
	...
	while (current_byte != uart_context->ctrl_char_scan_ptr)
	{
		...
		{
			current_byte = uart_context->rx_buffer
					+ (uart_context->rx_buffer_len - 1);
		}
	}
 ...
 ******************************************************************************/
static sl_status_t nolock_uart_write(void *context, const void *buffer,
 ...
 * Internal stream write implementation
 ******************************************************************************/
static sl_status_t uart_write(void *context, const void *buffer,
		size_t buffer_length)
{
#if (defined(SL_CATALOG_KERNEL_PRESENT))
  osStatus_t status;
 ...
 ******************************************************************************/
static sl_status_t uart_read(void *context, void *buffer, size_t buffer_length,
		size_t *bytes_read)
{
	CORE_DECLARE_IRQ_STATE;
	sl_iostream_uart_context_t *uart_context =
  ...
  if (osKernelGetState() == osKernelRunning) {
	...
	CORE_ENTER_ATOMIC();
	*bytes_read = read_rx_buffer(uart_context, (uint8_t*) buffer,
			(size_t) buffer_length);
	CORE_EXIT_ATOMIC();
...
dma_irq_handler(unsigned int chan, unsigned int seq, void *user_param)
{
	(void) seq;
	sl_iostream_uart_context_t *uart_context =
			(sl_iostream_uart_context_t*) user_param;
	uint8_t *write_ptr;
	Ecode_t ecode;
	...
	{
		available_space = (size_t) ((uart_context->rx_buffer
				+ uart_context->rx_buffer_len) - write_ptr);
	}
	...
	if (available_space > 0)
	{
		...
		ecode = DMADRV_PeripheralMemory(chan,
				uart_context->dma.cfg.peripheral_signal, write_ptr,
				uart_context->dma.cfg.src,
				true, available_space, dmadrvDataSize1, dma_irq_handler,
				uart_context);
		EFM_ASSERT(ecode == ECODE_OK);
	}
	// RX buffer is full
...
get_write_ptr(const sl_iostream_uart_context_t *uart_context)
{
  ...
  dst = ((uint8_t*)desc->DSTEND + 1) - remaining;
	...
	EFM_ASSERT(
			dst <= (uart_context->rx_buffer + uart_context->rx_buffer_len)
...
static void update_ring_buffer(sl_iostream_uart_context_t *uart_context)
{
	...
	if (write_ptr == uart_context->rx_read_ptr)
	{
		...
		ecode = DMADRV_PeripheralMemory(uart_context->dma.channel,
				uart_context->dma.cfg.peripheral_signal, write_ptr,
				uart_context->dma.cfg.src,
				true, uart_context->rx_buffer_len, dmadrvDataSize1,
				dma_irq_handler, uart_context);
		EFM_ASSERT(ecode == ECODE_OK);
	}
    ...
    if (uart_context->block) {
      ...
      if (osSemaphoreGetCount(uart_context->read_signal) == 0) {
		...
		if (dma_done)
		{
			...
			ecode = DMADRV_TransferCompletePending(uart_context->dma.channel,
					&irq_pending);
			EFM_ASSERT(ecode == ECODE_OK);
	...
	// Read data
	{
		...
		if (uart_context->sw_flow_control == true)
		{
			for (size_t bytes_read = 0; bytes_read < read_size; bytes_read++)
			{
				...
				if (*uart_context->rx_read_ptr == (uint8_t) UARTXON
						|| *uart_context->rx_read_ptr == (uint8_t) UARTXOFF)
				{
					// Current byte is the most recent scanned byte. Apply control character
					if (uart_context->rx_read_ptr
							== uart_context->ctrl_char_scan_ptr)
					{
						sl_atomic_store(uart_context->xon,
								(*uart_context->rx_read_ptr == (uint8_t)UARTXON));
					}
					ret_val--;
				}
			...
			if (uart_context->ctrl_char_scan_ptr
