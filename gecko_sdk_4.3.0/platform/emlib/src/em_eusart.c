...
static CMU_Clock_TypeDef
EUSART_ClockGet(EUSART_TypeDef *eusart);

static void
EUSART_AsyncInitCommon(EUSART_TypeDef *eusart,
...
static void
EUSART_SyncInitCommon(EUSART_TypeDef *eusart,
...
EUSART_AsyncBaudrateCalc(uint32_t refFreq, uint32_t clkdiv,
...
EUSART_Disable(EUSART_TypeDef *eusart);
		...
		const EUSART_UartInit_TypeDef *init)
{
	...
	EFM_ASSERT(
			EUSART_REF_VALID(eusart) && EUSART_EM2_CAPABLE(EUSART_NUM(eusart)));
	// Init structure must be provided.
	EFM_ASSERT(init);
    ...
    if (eusart == EUART0) {
      clock_source = CMU_ClockSelectGet(cmuClock_EUART0);
		...
		EFM_ASSERT(
				(clock_source == cmuSelect_ULFRCO) || (clock_source == cmuSelect_LFXO) || (clock_source == cmuSelect_LFRCO) || (clock_source == cmuSelect_EM23GRPACLK)
		...
		const EUSART_IrDAInit_TypeDef *irdaInit)
{
	...
	if (irdaInit->irDALowFrequencyEnable)
	{
		...
		// Number of stop bits can only be 1 or 2 in LF.
		EFM_ASSERT(
				(irdaInit->init.stopbits == eusartStopbits1)
						|| (irdaInit->init.stopbits == eusartStopbits2));
		// In LF, max baudrate is 9600. See Note #1.
		EFM_ASSERT(irdaInit->init.baudrate <= 9600);
		EFM_ASSERT(
				irdaInit->init.enable == eusartEnableRx
						|| irdaInit->init.enable == eusartDisable);
	}
	else
	{
		EFM_ASSERT(irdaInit->init.oversampling != eusartOVS0);
		...
		EFM_ASSERT(
				irdaInit->init.baudrate >= 2400
						&& irdaInit->init.baudrate <= 1152000);
	}
...
void EUSART_SpiInit(EUSART_TypeDef *eusart, EUSART_SpiInit_TypeDef const *init)
{
	...
	else
	{
		...
		if (init->advancedSettings && init->advancedSettings->forceLoad)
		{
			...
			EFM_ASSERT(
					(init->bitRate >= 5000000
							&& init->advancedSettings->setupWindow <= 4)
							|| (init->bitRate < 5000000
									&& init->advancedSettings->setupWindow >= 5));
		}
	}
		...
		const EUSART_DaliInit_TypeDef *daliInit)
{
	...
	if (daliInit->init.loopbackEnable)
	{
		...
		EFM_ASSERT(
				(daliInit->TXdatabits >> _EUSART_DALICFG_DALITXDATABITS_SHIFT) == (daliInit->RXdatabits >> _EUSART_DALICFG_DALIRXDATABITS_SHIFT));
	}
	...
	else
	{
		...
		EFM_ASSERT(
				daliInit->init.baudrate >= 1200
						&& daliInit->init.baudrate <= 57600);
	}
...
void EUSART_Enable(EUSART_TypeDef *eusart, EUSART_Enable_TypeDef enable)
{
	...
	else
	{
		...
		while ((eusart->STATUS
				& (_EUSART_STATUS_TXENS_MASK | _EUSART_STATUS_RXENS_MASK))
				!= tmp)
		{
		}
	}
...
void EUSART_Dali_Tx(EUSART_TypeDef *eusart, uint32_t data)
{
	...
	for (uint8_t index = 0; index < dali_tx_nb_packets[EUSART_NUM(eusart)];
			index++)
	{
...
uint32_t EUSART_Dali_Rx(EUSART_TypeDef *eusart)
{
	...
	for (uint8_t index = 0; index < dali_rx_nb_packets[EUSART_NUM(eusart)];
			index++)
	{
		// when DALICFG.DALIEN is set to 1, then all 16 bits [15:0] represent data
 ...
 ******************************************************************************/
void EUSART_BaudrateSet(EUSART_TypeDef *eusart, uint32_t refFreq,
		uint32_t baudrate)
{
	uint32_t clkdiv;
	...
	if (eusart->CFG0 & _EUSART_CFG0_SYNC_MASK)
	{
		...
		EFM_ASSERT(
				clkdiv <= (_EUSART_CFG2_SDIV_MASK >> _EUSART_CFG2_SDIV_SHIFT));
		...
		if (wasEnabled)
		{
			...
			if (eusart->STATUS
					& (_EUSART_STATUS_RXENS_MASK | _EUSART_STATUS_TXENS_MASK))
			{
		...
		eusart->CFG2 =
				(eusart->CFG2 & ~(_EUSART_CFG2_SDIV_MASK))
						| ((clkdiv << _EUSART_CFG2_SDIV_SHIFT)
		...
		const EUSART_PrsTriggerInit_TypeDef *init)
{
  ...
  PRS->CONSUMER_EUART0_TRIGGER = (init->prs_trigger_channel & _PRS_CONSUMER_EUART0_TRIGGER_MASK);
	...
	if (eusart == EUSART0)
	{
		PRS->CONSUMER_EUSART0_TRIGGER = (init->prs_trigger_channel
				& _PRS_CONSUMER_EUSART0_TRIGGER_MASK);
	}
#endif
#if defined(EUSART1)
	if (eusart == EUSART1)
	{
		PRS->CONSUMER_EUSART1_TRIGGER = (init->prs_trigger_channel
		...
		const EUSART_DaliInit_TypeDef *daliInit)
{
	...
	eusart->FRAMECFG = (eusart->FRAMECFG
			& ~(_EUSART_FRAMECFG_DATABITS_MASK | _EUSART_FRAMECFG_STOPBITS_MASK
	...
	eusart->CFG0 = (eusart->CFG0
			& ~(_EUSART_CFG0_OVS_MASK | _EUSART_CFG0_LOOPBK_MASK
	...
	if (init->advancedSettings)
	{
		eusart->CFG0 |= (uint32_t) init->advancedSettings->dmaHaltOnError
				<< _EUSART_CFG0_ERRSDMA_SHIFT;
		eusart->CFG0 |= (uint32_t) init->advancedSettings->txAutoTristate
				<< _EUSART_CFG0_AUTOTRI_SHIFT;
		eusart->CFG0 |= (uint32_t) init->advancedSettings->invertIO
				& (_EUSART_CFG0_RXINV_MASK | _EUSART_CFG0_TXINV_MASK);
		eusart->CFG0 |= (uint32_t) init->advancedSettings->collisionDetectEnable
				<< _EUSART_CFG0_CCEN_SHIFT;
		eusart->CFG0 |= (uint32_t) init->advancedSettings->multiProcessorEnable
				<< _EUSART_CFG0_MPM_SHIFT;
		eusart->CFG0 |=
				(uint32_t) init->advancedSettings->multiProcessorAddressBitHigh
						<< _EUSART_CFG0_MPAB_SHIFT;
		eusart->CFG0 |= (uint32_t) init->advancedSettings->msbFirst
				<< _EUSART_CFG0_MSBF_SHIFT;

		// Configure global configuration register 1.
		eusart->CFG1 = (uint32_t) init->advancedSettings->dmaWakeUpOnRx
				<< _EUSART_CFG1_RXDMAWU_SHIFT
				| (uint32_t) init->advancedSettings->dmaWakeUpOnTx
						<< _EUSART_CFG1_TXDMAWU_SHIFT;

		if (init->advancedSettings->hwFlowControl == eusartHwFlowControlCts
				|| init->advancedSettings->hwFlowControl
      ...
      GPIO->EUARTROUTE_SET->ROUTEEN = GPIO_EUART_ROUTEEN_RTSPEN;
			...
			GPIO->EUSARTROUTE_SET[EUSART_NUM(eusart)].ROUTEEN =
			GPIO_EUSART_ROUTEEN_RTSPEN;
#endif
		}
		else
		{
...
#elif defined(EUSART0)
			GPIO->EUSARTROUTE_CLR[EUSART_NUM(eusart)].ROUTEEN =
			GPIO_EUSART_ROUTEEN_RTSPEN;
#endif
		}
		eusart->STARTFRAMECFG_SET =
				(uint32_t) init->advancedSettings->startFrame;
      ...
      PRS->CONSUMER_EUART0_RX_SET = (init->advancedSettings->prsRxChannel & _PRS_CONSUMER_EUART0_RX_MASK);
			...
			if (eusart == EUSART0)
			{
				PRS->CONSUMER_EUSART0_RX_SET =
						(init->advancedSettings->prsRxChannel
								& _PRS_CONSUMER_EUSART0_RX_MASK);
			}
#if defined(EUSART1)
			if (eusart == EUSART1)
			{
				PRS->CONSUMER_EUSART1_RX_SET =
						(init->advancedSettings->prsRxChannel
								& _PRS_CONSUMER_EUSART1_RX_MASK);
			}
#endif
#if defined(EUSART2)
      if (eusart == EUSART2) {
	...
	if (irdaInit)
	{
		...
		else
		{
			...
			eusart->IRHFCFG_SET = (eusart->IRHFCFG
	...
	if (daliInit)
	{
		...
		eusart->DALICFG = (eusart->DALICFG
				& ~(_EUSART_DALICFG_DALITXDATABITS_MASK
						| _EUSART_DALICFG_DALIRXDATABITS_MASK))
				| daliInit->TXdatabits | daliInit->RXdatabits;
		...
		EUSART_SpiInit_TypeDef const *init)
{
	...
	eusart->CFG2 = (eusart->CFG2
			& ~(_EUSART_CFG2_MASTER_MASK | _EUSART_CFG2_CLKPOL_MASK
					| _EUSART_CFG2_CLKPHA_MASK | _EUSART_CFG2_FORCELOAD_MASK))
			| (uint32_t) (init->master) | (uint32_t) (init->clockMode)
	...
	if (advancedSetting_ptr)
	{
		// Configure global configuration register 2.
		eusart->CFG2 = (eusart->CFG2
				& ~(_EUSART_CFG2_FORCELOAD_MASK | _EUSART_CFG2_AUTOCS_MASK
						| _EUSART_CFG2_AUTOTX_MASK | _EUSART_CFG2_CSINV_MASK
						| _EUSART_CFG2_CLKPRSEN_MASK))
				| (uint32_t) (init->advancedSettings->forceLoad
						<< _EUSART_CFG2_FORCELOAD_SHIFT)
				| (uint32_t) (init->advancedSettings->autoCsEnable
						<< _EUSART_CFG2_AUTOCS_SHIFT)
				| (uint32_t) (init->advancedSettings->autoTxEnable
						<< _EUSART_CFG2_AUTOTX_SHIFT)
				| (uint32_t) (init->advancedSettings->csPolarity)
				| (uint32_t) (init->advancedSettings->prsClockEnable
						<< _EUSART_CFG2_CLKPRSEN_SHIFT);

		// Only applicable to EM2 (low frequency) capable EUSART instances.
		eusart->CFG1 = (eusart->CFG1
				& ~(_EUSART_CFG1_RXFIW_MASK | _EUSART_CFG1_TXFIW_MASK))
				| (uint32_t) (init->advancedSettings->RxFifoWatermark)
				| (uint32_t) (init->advancedSettings->TxFifoWatermark)
				| (uint32_t) (init->advancedSettings->dmaWakeUpOnRx
						<< _EUSART_CFG1_RXDMAWU_SHIFT)
				| (uint32_t) (init->advancedSettings->prsRxEnable
						<< _EUSART_CFG1_RXPRSEN_SHIFT);
	}

	eusart->CFG0 = (eusart->CFG0
			& ~(_EUSART_CFG0_SYNC_MASK | _EUSART_CFG0_LOOPBK_MASK))
	...
	if (advancedSetting_ptr)
	{
		...
		eusart->TIMINGCFG = (eusart->TIMINGCFG
				& ~(_EUSART_TIMINGCFG_CSSETUP_MASK
						| _EUSART_TIMINGCFG_CSHOLD_MASK
						| _EUSART_TIMINGCFG_ICS_MASK
						| _EUSART_TIMINGCFG_SETUPWINDOW_MASK))
				| ((uint32_t) (init->advancedSettings->autoCsSetupTime
						<< _EUSART_TIMINGCFG_CSSETUP_SHIFT)
						& _EUSART_TIMINGCFG_CSSETUP_MASK)
				| ((uint32_t) (init->advancedSettings->autoCsHoldTime
						<< _EUSART_TIMINGCFG_CSHOLD_SHIFT)
						& _EUSART_TIMINGCFG_CSHOLD_MASK)
				| ((uint32_t) (init->advancedSettings->autoInterFrameTime
						<< _EUSART_TIMINGCFG_ICS_SHIFT)
						& _EUSART_TIMINGCFG_ICS_MASK)
				| ((uint32_t) (init->advancedSettings->setupWindow
						<< _EUSART_TIMINGCFG_SETUPWINDOW_SHIFT)
						& _EUSART_TIMINGCFG_SETUPWINDOW_MASK);
	}
	...
	if (advancedSetting_ptr)
	{
		eusart->DTXDATCFG = (init->advancedSettings->defaultTxData
				& _EUSART_DTXDATCFG_MASK);
      ...
      if (eusart == EUSART4) {
		...
		if (init->advancedSettings->prsClockEnable)
		{
			...
			if (eusart == EUSART0)
			{
				PRS->CONSUMER_EUSART0_CLK_SET =
						(init->advancedSettings->prsClockChannel
								& _PRS_CONSUMER_EUSART0_CLK_MASK);
			}
#if defined(EUSART1)
			if (eusart == EUSART1)
			{
				PRS->CONSUMER_EUSART1_CLK_SET =
						(init->advancedSettings->prsClockChannel
								& _PRS_CONSUMER_EUSART1_CLK_MASK);
			}
#endif
#if defined(EUSART2)
      if (eusart == EUSART2) {
...
__STATIC_INLINE uint32_t EUSART_AsyncBaudrateCalc(uint32_t refFreq,
		uint32_t clkdiv, EUSART_OVS_TypeDef ovs)
{
	uint32_t oversample;
	uint64_t divisor;
	uint64_t factor;
...
#if defined(EUSART_PRESENT)
		if (!(eusart->CFG0 & _EUSART_CFG0_SYNC_MASK)
				|| (eusart->CFG2 & _EUSART_CFG2_MASTER_MASK))
#endif
		{
			...
			eusart_sync(eusart,
					(EUSART_SYNCBUSY_TXDIS | EUSART_SYNCBUSY_RXDIS));
