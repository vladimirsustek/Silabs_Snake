...
{
	GPIO_PinOutSet(SL_MX25_FLASH_SHUTDOWN_CS_PORT,
	SL_MX25_FLASH_SHUTDOWN_CS_PIN);
}
#endif
...
void sl_mx25_flash_shutdown(void)
{
	...
	// IO config
	GPIO_PinModeSet(SL_MX25_FLASH_SHUTDOWN_TX_PORT,
	SL_MX25_FLASH_SHUTDOWN_TX_PIN, gpioModePushPull, 1);
	GPIO_PinModeSet(SL_MX25_FLASH_SHUTDOWN_RX_PORT,
	SL_MX25_FLASH_SHUTDOWN_RX_PIN, gpioModeInput, 0);
	GPIO_PinModeSet(SL_MX25_FLASH_SHUTDOWN_CLK_PORT,
	SL_MX25_FLASH_SHUTDOWN_CLK_PIN, gpioModePushPull, 1);
	GPIO_PinModeSet(SL_MX25_FLASH_SHUTDOWN_CS_PORT,
	SL_MX25_FLASH_SHUTDOWN_CS_PIN, gpioModePushPull, 1);

#ifdef _GPIO_USART_ROUTEEN_MASK
	GPIO->USARTROUTE[SL_MX25_FLASH_SHUTDOWN_PERIPHERAL_NO].CLKROUTE =
			((SL_MX25_FLASH_SHUTDOWN_CLK_PORT << _GPIO_USART_CLKROUTE_PORT_SHIFT)
					| (SL_MX25_FLASH_SHUTDOWN_CLK_PIN
							<< _GPIO_USART_CLKROUTE_PIN_SHIFT));
	GPIO->USARTROUTE[SL_MX25_FLASH_SHUTDOWN_PERIPHERAL_NO].RXROUTE =
			((SL_MX25_FLASH_SHUTDOWN_RX_PORT << _GPIO_USART_RXROUTE_PORT_SHIFT)
					| (SL_MX25_FLASH_SHUTDOWN_RX_PIN
							<< _GPIO_USART_RXROUTE_PIN_SHIFT));
	GPIO->USARTROUTE[SL_MX25_FLASH_SHUTDOWN_PERIPHERAL_NO].TXROUTE =
			((SL_MX25_FLASH_SHUTDOWN_TX_PORT << _GPIO_USART_TXROUTE_PORT_SHIFT)
					| (SL_MX25_FLASH_SHUTDOWN_TX_PIN
							<< _GPIO_USART_TXROUTE_PIN_SHIFT));
	GPIO->USARTROUTE[SL_MX25_FLASH_SHUTDOWN_PERIPHERAL_NO].ROUTEEN =
  ...
  SL_MX25_FLASH_SHUTDOWN_PERIPHERAL->ROUTEPEN  = (USART_ROUTEPEN_RXPEN
	...
	// Deinit flash
	GPIO_PinModeSet(SL_MX25_FLASH_SHUTDOWN_TX_PORT,
	SL_MX25_FLASH_SHUTDOWN_TX_PIN, gpioModeDisabled, 0);
	GPIO_PinModeSet(SL_MX25_FLASH_SHUTDOWN_RX_PORT,
	SL_MX25_FLASH_SHUTDOWN_RX_PIN, gpioModeDisabled, 0);
	GPIO_PinModeSet(SL_MX25_FLASH_SHUTDOWN_CLK_PORT,
	SL_MX25_FLASH_SHUTDOWN_CLK_PIN, gpioModeDisabled, 1);
	GPIO_PinModeSet(SL_MX25_FLASH_SHUTDOWN_CS_PORT,
	SL_MX25_FLASH_SHUTDOWN_CS_PIN, gpioModeDisabled, 1);

	USART_Reset(SL_MX25_FLASH_SHUTDOWN_PERIPHERAL);

#ifdef _GPIO_USART_ROUTEEN_MASK
	GPIO->USARTROUTE[SL_MX25_FLASH_SHUTDOWN_PERIPHERAL_NO].CLKROUTE =
	_GPIO_USART_CLKROUTE_RESETVALUE;
	GPIO->USARTROUTE[SL_MX25_FLASH_SHUTDOWN_PERIPHERAL_NO].RXROUTE =
	_GPIO_USART_RXROUTE_RESETVALUE;
	GPIO->USARTROUTE[SL_MX25_FLASH_SHUTDOWN_PERIPHERAL_NO].TXROUTE =
	_GPIO_USART_TXROUTE_RESETVALUE;
	GPIO->USARTROUTE[SL_MX25_FLASH_SHUTDOWN_PERIPHERAL_NO].ROUTEEN =
